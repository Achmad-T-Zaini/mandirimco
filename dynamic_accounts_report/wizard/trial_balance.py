...
class TrialView(models.TransientModel):
    ...
    def view_report(self, option):
        ...
        if todayDate.day > 25:
            firstday -=relativedelta(months=1)

        lastday = firstday + relativedelta(months=1, days=-1)
    ...
    def _get_accounts(self, accounts, display_account, data):
        ...
        request = (
                    ...
                    " FROM " + tables + " join account_move am on am.id=account_move_line.move_id WHERE account_id IN %s " + filters + " GROUP BY account_id")
        params = (tuple(accounts.ids),) + tuple(where_params)
#        raise UserError(_('request %s \n param %s')%(request,params,))
    ...
    def get_init_bal(self, account, display_account, data):
        if data.get('date_from'):
            ...
            request = (
                    "SELECT account_id AS id, SUM(debit) AS debit, SUM(credit) AS credit, (SUM(debit) - SUM(credit)) AS balance" + \
                    " FROM " + tables + " join account_move am on am.id=account_move_line.move_id WHERE account_id = %s" % account.id + filters + " GROUP BY account_id")
